...
export default function SectionWorkspace({ section, action }: { section: string; action?: string }) {
  ...
  return <><div className="section-action-row"><button className="clay-button button-dark" type="button" onClick={openAdd}><Plus size={16} /> {actions[section]}</button></div><SectionList section={section} reloadKey={reloadKey} onEdit={(record) => setModal({ mode: "edit", record })} onDeleted={(message) => { setReloadKey((value) => value + 1); setNotice(message); window.setTimeout(() => setNotice(""), 3000); }} />{notice && <div className="success-toast" role="status">{notice}</div>}{modal && <AdminRecordModal section={section} mode={modal.mode} record={modal.record} onClose={closeModal} onSaved={saved} />}</>;
}
